...
| Syntax | Syntax highlighting | Partial | `src/renderables/syntax.rs` (feature `syntax`) | Python fixture conformance (`syntax/basic`, `syntax/no_terminal`) now checks ANSI directly and passes for covered Rust defaults via `python-rich-default` compatibility theme behavior. Custom syntect theme/syntax loading is implemented (`Syntax::theme_set`, `Syntax::syntax_set`, `load_themes_from_folder`, `load_syntaxes_from_folder`) and covered by `tests/e2e_syntax.rs`. |
